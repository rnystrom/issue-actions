...
function makeRepositories(arr: ReadonlyArray < string > ): ReadonlyArray < Repository > {
...
function makeLabels(str: string): ReadonlyArray < LabelConfig > {
  var labels = new Array < LabelConfig > ()
  ...
  for (var i in jsonArray) {
    ...
    const addLabels = jsonArray[i].addLabels == null ? Array < string > () : jsonArray[i].addLabels
...
const run = async (): Promise < void > => {
  ...
  const parentMarker = makeParentMarker(github.context.payload.repository.owner.login, github.context.payload.repository.name, issue.number)
  ...
  var updateLabels: Array < Label > = issue.labels
  var prependChecklists = new Array < string > ()
  ...
  for (var i in labels) {
    ...
    for (var j in label.repositories) {
      ...
      const marker = makeChecklistMarker(label, repo)
      ...
      try {
        ...
        prependChecklists.push(makeChecklist(issueCreateResponse.data.html_url, false) + ` ${marker}`)
      } catch (error) {
